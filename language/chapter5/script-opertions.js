...
//once node js setup is done replace the array with require it from other file
var SCRIPTS = [
    {
        name: "Adlam",
...
//console.log("scripts with direction from right to left are :" , rtlNames);
...
let ltrNames = map(filter(SCRIPTS , s=>s.direction==='ltr') , s => s.name);


//console.log("scripts with direction from left  to right are :" , reduce(ltrNames ,( n , c )=> n +" " + c , " "));
//console.log(reduce([1, 2, 3, 4], (a, b) => a + b, 0));

//to find the script with the most characters, we can write something like this:
...
let scriptWithMaxChars = SCRIPTS.reduce((a, b) =>  characterCount(a) < characterCount(b) ? b : a);
//console.log("name of script with max chars is : ", map([scriptWithMaxChars], s => s.name) , " and count of chars is :" , characterCount(scriptWithMaxChars));
...
//console.log("script name containing code 121 is", map([characterScript(121)], s=>s.name));
